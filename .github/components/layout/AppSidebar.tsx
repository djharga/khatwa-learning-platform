...
const AppSidebar = ({ disabled = false }: AppSidebarProps) => {
  ...
  const navigationItems = getSidebarItems(
    ...
    isAuthenticatedUser // تمرير حالة المصادقة
  );
          ...
          <Link
            ...
            className={`group flex items-center gap-3.5 p-3.5 rounded-xl text-sm transition-all duration-[200ms] ease-out min-h-[44px] ${
              active
                ...
                : 'text-neutral-700 hover:bg-neutral-100 hover:text-primary-600 border border-transparent focus-visible:outline-2 focus-visible:outline-primary-500 focus-visible:outline-offset-2'
            }`}
          ...
          >
            <IconComp
              className={`w-[18px] h-[18px] transition-colors duration-[200ms] ease-out ${
                active ? 'text-white' : 'text-neutral-600 group-hover:text-primary-500'
              }`}
              strokeWidth={2}
              aria-hidden="true"
            />
  ...
  const CategorySection = ({ section, expanded, toggle, idx }: any) => (
    ...
    >
      <motion.button
        onClick={toggle}
        className={`w-full flex items-center justify-between p-4 rounded-xl text-sm font-bold text-neutral-900 border transition-all duration-[200ms] ease-out min-h-[44px] focus-visible:outline-2 focus-visible:outline-primary-500 focus-visible:outline-offset-2 ${
          expanded 
  ...
  const SidebarHeader = ({ onClose }: { onClose: () => void }) => (
    <div className="flex items-center justify-between p-5 border-b border-neutral-200 bg-white">
      <h2 className="text-xl font-extrabold text-primary-600 tracking-tight">
        خطي التعليمية
      </h2>
      <motion.button
        onClick={onClose}
      ...
      <>
        ...
        {isOpen && (
          <motion.aside
            ...
            className="hidden lg:flex flex-col fixed start-0 top-16 h-[calc(100vh-4rem)] w-80 border-e border-neutral-200 z-[60] rounded-r-2xl shadow-elevation-5"
            style={{
  ...
  // إذا لم يكن هناك عناصر تنقل، نعرض على الأقل زر التبديل
  // لكن نعرض AppSidebar دائماً حتى لو كان فارغاً
